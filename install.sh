...
install_dependencies() {
    dependencies="sway swaybg waybar alacritty wofi grim slurp pulseaudio-utils brightnessctl zip swayidle swaylock playerctl pamixer autotiling thunar neovim xdg-desktop-portal xdg-desktop-portal-wlr"
...
for source_dir in "$base_dir"/*; do
    ...
    # If the current config being processed is 'sway', give execute permissions to scripts
    if [ "$config_name" = "sway" ]; then
        find "$target_dir/scripts" -type f -exec chmod +x {} \; || true
    fi
done
